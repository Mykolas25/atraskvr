...
@section('content')
        ...
        @foreach($pagesShow as $pagedata)
            ...
            <div class="grid">
                <div class="col-md-12">
                    ...
                    <div class="col-md-2"><img src="{{URL::asset($pagedata['resource_image']['path'])}}" alt="Forest"
                                               width="80" height="160"/></div>
                    <div class="col-md-2"><a class="btn btn-primary"
                                             href="{{route('app.pages.mediaFiles', $pagedata['id'])}}">Media files</a>
                    </div>
                    <div class="col-md-2"><a class="btn btn-success"
                                             href="{{route('app.pages.edit', $pagedata['id'])}}">Edit</a></div>
                    <div class="col-md-2"><a class="btn btn-warning"
                                             href="{{route('app.pages.delete', $pagedata['id'])}}">Delete</a></div>
                </div>
            </div>
    ...
    @if(isset($tableName))
        ...
        <div class="standardDiv">{!! Form::submit('Create' , ['class' => 'btn btn-success']) !!}
            <a class="btn btn-primary" href="{{ route('app.' . $tableName . '.show') }}">{{ucfirst($tableName)}}
                list</a>
        </div>
        {!! Form::close() !!}
